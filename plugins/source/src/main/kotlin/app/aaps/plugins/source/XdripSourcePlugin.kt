...
class XdripSourcePlugin @Inject constructor(
    ...
    class XdripSourceWorker(
        ...
        override suspend fun doWorkAndLog(): Result {
            ...
            val xdripOM = preferences.get(BooleanKey.OApsxdriponeminute)
            val maxTs = bundles.maxOfOrNull { it.getLong(Intents.EXTRA_TIMESTAMP, 0) } ?: 0L
            if (!xdripOM && maxTs > 0 && lastDataTimestamp > 0 && maxTs - lastDataTimestamp < 240_000) {
                return Result.success(workDataOf("Result" to "Ignoring data, not enough time passed since last processing"))
            }
            if (maxTs > 0) {
                lastDataTimestamp = maxTs
            }
